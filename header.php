...
<html <?php language_attributes(); ?>>
    ...
    <body <?php body_class(); ?>>
        ...
        <?php
            ...
            $on_front_page          = get_theme_mod( 'techmoon-header-front-page', true );
            ...
            // blog page ( show / hide )
            $on_blog_page           = get_theme_mod( 'techmoon-header-blog-page', false );
            ...
            // check if is single post
                ...
                $show_header = get_theme_mod( 'techmoon-header-single-post', false );
            }
            ...
            // check if is single page ( exclude static front page )
                ...
                $show_header = get_theme_mod( 'techmoon-header-single-page', false );
            }

            // show or hide the header for archives templates
            else{
                $show_header = get_theme_mod( 'techmoon-header-templates', false );
            }

            // header class
            $techmoon_header_class      = 'techmoon-miss-header-image';

            if( $show_header ){
                $techmoon_header_class  = '';
            }
        ?>
        ...
        <?php
             ...
             */
        ?>
		<div class="techmoon-noticebar">

		</div>
        <header class="<?php echo esc_attr( $techmoon_header_class ); ?>">

		<nav class="white techmoon-topper" role="navigation">
                <div class="nav-wrapper container siteidentity">
                    ...
                    <?php
                        ...
                        $techmoon_logo_src              = esc_url( get_theme_mod( 'techmoon-blog-logo' , get_template_directory_uri() . '/media/_frontend/img/logo.png' ) );

                        echo '<div class="techmoon-blog-identity">';

                        if ( function_exists( 'the_custom_logo' ) && $has_custom_logo = has_custom_logo() ) {
                            echo '<div class="techmoon-blog-logo" style="margin-top: ' . absint( get_theme_mod( 'techmoon-blog-logo-m-top' ) ) . 'px; margin-bottom: ' . absint( get_theme_mod( 'techmoon-blog-logo-m-bottom' ) ) . 'px;">';
                            the_custom_logo();
                            echo '</div>';
                        }

                        // blog logo ( if not is empty )
                        else if( !empty( $techmoon_logo_src ) ){
                            echo '<a class="techmoon-blog-logo" href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( $blog_title . ' - ' . $blog_description ) . '" style="margin-top: ' . absint( get_theme_mod( 'techmoon-blog-logo-m-top' ) ) . 'px; margin-bottom: ' . absint( get_theme_mod( 'techmoon-blog-logo-m-bottom' ) ) . 'px;">';
                            echo '<img src="' . esc_url( $techmoon_logo_src ) . '" title="' . esc_attr( $blog_title . ' - ' . $blog_description ) . '"/>';
                            echo '</a>';
                        }

                        // blog title ( if not is empty )
                        if( !empty( $blog_title ) ){
                            echo '<a class="techmoon-blog-title" href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( $blog_title . ' - ' . $blog_description ) . '">';
                            bloginfo( 'name' );
                            echo '</a>';
                        }
                        ...
                        if( !empty( $blog_description ) ){
                            echo '<a class="techmoon-blog-description" href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( $blog_title . ' - ' . $blog_description ) . '">';
...
?>
                       ...
                       <div class="parallexheaderimage"></div> 
            </nav>

            <!-- header navigation -->
            <nav class="techmoon-topper sitemenu" role="navigation">
                <div class="nav-wrapper container">
				<?php
                        global $techmoon_curr_ancestor;
                             ...
                             */
                                ...
                                if( !empty( $pages ) ){
                                    ...
                                    foreach( $pages as $p => $item ){
                                        ...
                                        $submenu = techmoon_tools::submenu( $item -> ID );

                                        if( !empty( $submenu ) ){
                                            $classes .= ' menu-item-has-children';

                                            if( $techmoon_curr_ancestor  ){
                                                $classes .= ' current-menu-ancestor';
                                            }
                                        }

                                        echo '<li class="menu-item ' . $classes . '">';
                                        echo '<a href="' . esc_url( get_permalink( $item -> ID ) ) . '" title="' . esc_attr( techmoon_post::title( $item -> ID, true ) ) . '">' . techmoon_post::title( $item -> ID ) . '</a>';
                                        echo $submenu;
                                        echo '</li>';
                                    }
                                    echo '</ul>';
