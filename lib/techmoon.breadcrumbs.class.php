<?php
if( !class_exists( 'techmoon_breadcrumbs' ) ){

class techmoon_breadcrumbs
{
	static function home()
    {
        $rett  = '<li id="home-label">';
        $rett .= '<a href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( get_theme_mod( 'techmoon-breadcrumbs-home-link-description' , __( 'go to home' , 'materialize' ) ) ) . '">';
        $rett .= '<i class="materialize-icon-home"></i> <span>' . esc_html( get_theme_mod( 'techmoon-breadcrumbs-home-link-text' , __( 'Home' , 'materialize' ) ) ) . '</span>';
        $rett .= '</a>';
        $rett .= '</li>';
	...
	static function pages( $p )
	{
        ...
        if( !is_page( $p -> ID  ) ){
            $rett .= '<li>';
            $rett .= '<a href="' . get_permalink( $p -> ID ) . '" title="' . techmoon_post::title( $p -> ID, true ) . '">' . techmoon_post::title( $p -> ID, true )  . '</a>';
            $rett .= '</li>';
        }
