<?php
if( !class_exists( 'techmoon_tools' ) ){
    
class techmoon_tools
{
    /* WEB RESOURCES */
    ...
    static function submenu( $id )
    {
        ...
        if( !empty( $pages ) ){
            ...
            foreach( $pages as $p => $item ){
                ...
                $rett .= '<a href="' . esc_url( get_permalink( $item -> ID ) ) . '" title="' . techmoon_post::title( $item -> ID, true ) . '">' . techmoon_post::title( $item -> ID ) . '</a>';
