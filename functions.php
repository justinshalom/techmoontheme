 ...
 */
	...
	add_action( 'admin_init', 			array( 'techmoon_scripts', 	'backend' ) );

	add_action( 'admin_menu', 			array( 'techmoon_admin', 	'pageMenu' ) );

	add_action( 'widgets_init', 		array( 'techmoon_setup', 	'sidebars' ) );
	add_action( 'init', 				array( 'techmoon_setup', 	'menus' ) );

	add_action( 'wp_enqueue_scripts',  array( 'techmoon_scripts',  'frontend' ), 0 );
	add_action( 'wp_head', 				array( 'techmoon_header', 	'head' ) );

    add_filter( 'the_excerpt_rss', 		array( 'techmoon_tools', 	'rss_with_thumbnail' ) );
    add_filter( 'the_content_feed', 	array( 'techmoon_tools', 	'rss_with_thumbnail' ) );

    if( get_theme_mod( 'techmoon-gallery', true ) ){
        add_action( 'admin_init',           array( 'techmoon_gallery',  'admin_init' ) );
        add_filter( 'post_gallery',         array( 'techmoon_gallery',  'shortcode'), null, 2 );
    }
    ...
    add_image_size( 'techmoon-classic' , 1140 , 515 , true );
